...
# ──────────────────────────────────────────────
@extend_schema_view(
    get = extend_schema(tags = ['Tipo Mascota']),
    post = extend_schema(tags = ['Tipo Mascota']),
)
...
@extend_schema_view(
    get = extend_schema(tags = ['Tipo Mascota']),
    put = extend_schema(tags = ['Tipo Mascota']),
    delete = extend_schema(tags = ['Tipo Mascota']),
)
...
# ──────────────────────────────────────────────
@extend_schema_view(
    get = extend_schema(tags = ['Mascotas']),
    post = extend_schema(tags = ['Mascotas'])
)
class MascotaListView(APIView):
    ...
    def get(self, request):
        mascotas = Mascotas.objects.filter(owner=self.request.user if self.request.user.is_authenticated else User.objects.first())
...
@extend_schema_view(
    get = extend_schema(tags = ['Mascotas']),
    put = extend_schema(tags = ['Mascotas']),
    delete = extend_schema(tags = ['Mascotas'])
)
class MascotaDetailView(APIView):
    ...
    def put(self, request, pk):
        mascota, error = self._get_mascota_owner(pk, self.request.user if self.request.user.is_authenticated else User.objects.first())
        if error:
            return error
